...
def vllm_infer(
    ...
    if(save_name is None):
        save_name = f"{int(time.time())}_generated_predictions.jsonl"

    # Add batch process to avoid the issue of too many files opened
    for i in range(0, len(train_dataset), batch_size):
        print(f"Processing batched inference: {i}/{len(train_dataset)}")
        ...
        print(f"The {len(prompts)} results generated by the current batch have been saved at {save_name}.")
