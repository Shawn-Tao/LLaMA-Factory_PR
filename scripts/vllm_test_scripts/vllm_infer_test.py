    ...
    # for i in tqdm(range(0, len(train_dataset), batch_size), desc="Processing batches"):   
        ...
        for j in range(len(batch["input_ids"])):
            if batch["images"][j] is not None:
                ...
                regularized_images = template_obj.mm_plugin._regularize_images(image, image_max_pixels=image_max_pixels, image_min_pixels=image_min_pixels)["images"]
                input_image_list = []
                for i in range(len(regularized_images)):
                    image_array = np.array(regularized_images[i])
                    input_image_list.append(image_array)
                multi_modal_data = {"image": input_image_list}
                
                # multi_modal_data = {"image": template_obj.mm_plugin._regularize_images(image, image_max_pixels=image_max_pixels, image_min_pixels=image_min_pixels)["images"]}
